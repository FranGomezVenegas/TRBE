...
/**
 * LPPlatform is a library for methods solving topics that are specifically part
 * of the LabPLANET Paradigm.
 *
 * @author Fran Gomez
 */
public class LPPlatform {
    ...
    public static Boolean isEncryptedField(String schemaName, String areaName, String tableName, String fieldName) {
        ...
        if ((schemaName == null) || (tableName == null) || (fieldName == null)) {
            return diagnoses;
        }
        ...
        if ((tableEncrytedFields == null)) {
            return diagnoses;
        }
        if (("".equals(tableEncrytedFields))) {
            return diagnoses;
        }
        ...
        if (Boolean.FALSE.equals(fieldName.toUpperCase().contains("IN"))) {
            ...
            for (String fn : valuesArr) {
                ...
                if (Boolean.TRUE.equals(override)) {
                    ...
                    if (newFieldValueBuilder.length() > 0) {
                        newFieldValueBuilder.append(separator);
                    }
    ...
    /**
     * addJavaClassDoc is the method that should reduce the lines of code for
     * justifying lines of code against its requirement to keep the track about
     * which is the requirement covered by each section in each method. When
     * running the code in Dev-Mode then it should mark as 'covered' the
     * existing requirement or create one record for this given requirement The
     * parameter.config.testing-html-settings
     * mandatoryFields_requerimentsJavaDoc defines which are the mandatory
     * fields that should be added to the peer fields/values to let this call be
     * consider fill enough to proceed.
     *
     * @param fields String[] - which are the properties being passed.
     * @param values Object[] - which are the values for the properties defined
     * above
     * @param elementsDev StackTraceElement[] - Provides info from the context
     * such as the ClassName + MethodName + LineNumber
     */
    public static void addJavaClassDoc(String[] fields, Object[] values, StackTraceElement[] elementsDev) {
        ...
        fldName = LPArray.addValueToArray1D(fldName, ApiMessageReturn.JAVADOC_CLASS_FLDNAME);
        fldValue = LPArray.addValueToArray1D(fldValue, elementsDev[1].getClassName());
        fldName = LPArray.addValueToArray1D(fldName, ApiMessageReturn.JAVADOC_METHOD_FLDNAME);
        fldValue = LPArray.addValueToArray1D(fldValue, elementsDev[1].getMethodName());
        fldName = LPArray.addValueToArray1D(fldName, ApiMessageReturn.JAVADOC_LINE_FLDNAME);
        fldValue = LPArray.addValueToArray1D(fldValue, elementsDev[1].getLineNumber());

        for (Integer iNumFields = 0; iNumFields < fields.length; iNumFields++) {
            if ((fields[iNumFields] != null) && (values[iNumFields] != null)) {
                fldName = LPArray.addValueToArray1D(fldName, fields[iNumFields]);
                fldValue = LPArray.addValueToArray1D(fldValue, values[iNumFields]);
            }
        }

        String[] getFilterFldName = new String[0];
        Object[] getFilterFldValue = new Object[0];
        getFilterFldName = LPArray.addValueToArray1D(getFilterFldName, ApiMessageReturn.JAVADOC_CLASS_FLDNAME);
        getFilterFldValue = LPArray.addValueToArray1D(getFilterFldValue, elementsDev[1].getClassName());
        getFilterFldName = LPArray.addValueToArray1D(getFilterFldName, ApiMessageReturn.JAVADOC_METHOD_FLDNAME);
        getFilterFldValue = LPArray.addValueToArray1D(getFilterFldValue, elementsDev[1].getMethodName());
...
*/        
    ...
    /**
     * The schema names are instances per procedure + nature of the data
     * (config/data/requirements...) This method has as a purpose on helping on
     * build the concatenation At the same time it solves the problem on using
     * some symbols like "-" in the name that requires quoted the name If the
     * schemaName is already contained in the procInstanceName it won't be
     * concatenated again.
     *
     * @param procInstanceName String - Basically the Procedure Name
     * @param schemaName String - Which is the nature of the data
    ...
    /**
     * When logging/creating objects that conceptually are mandatory on be part
     * of a structure for a field added or required to get all the fields
     * consider mandatory we invoke the specific parameter field called in the
     * way of "table_name_mandatoryFieldsAction" containing a peer entries in
     * the way of: A call per each mandaotry field to the method
     * mandatoryFieldsByDependency will add the prerrequisites as mandatory too
     * All fields should be in context when the action is performed and not
     * null.The entry is stored in the specific data.properties file for this
     * particular procedure. where the content is expressed in the way of
     * fieldNAmes between spaces where the first field is the one having the
     * prerrequisites. and all different fields separated by pipe, "|". Example:
     * project_fieldsAddingMandatoryInsert:analysis method_name
     * method_version*analysis method_name method_version|method_name*analysis
     * method_name method_version|spec*spec spec_code spec_code_version
     *
     * @param procInstanceName - Schema where the template belongs to
     * @param areaName
     * @param fieldNames[] - Fields for the filter to find and get the
     * prerrequisites.
     * @param fieldValues
     * @param tableName. Table where the template is stored in.
     * @param actionName. The action in the database INSERT/UPDATE/DELETE
     * (lowercas preferred).
     * @return String[] All prerrequisite fields for all the fields added to the
     * fieldNames input argument array, when position 3 is set to FALSE then the
    ...
    public static Object[][] mandatoryFieldsCheck(String procInstanceName, String areaName, String[] fieldNames, Object[] fieldValues, String tableName, String actionName) {
            ...
            if ((Boolean.FALSE.equals(contains)) || (fieldValues[inumLines] == null)) {
                ...
                if (Boolean.FALSE.equals(addIt)) {
                    if (mandatoryFieldsMissing.length() > 0) {
                        mandatoryFieldsMissing = mandatoryFieldsMissing.append(",");
                    }
        ...
        for (Integer i = 0; i < fieldNames.length; i++) {
            if (i >= 6) {
                diagnoses = LPArray.addColumnToArray2D(diagnoses, "");
            }
            diagnoses[1][i] = fieldNames[i];
        }
        for (Integer i = 0; i < fieldNames.length; i++) {
            diagnoses[2][i] = fieldValues[i];
        }
     ...
     * When logging/creating objects that conceptually are mandatory on be part
     * of a structure for a field added or required to get all the fields
     * involved per field we invoke the specific parameter field called in the
     * way of "table_name_fieldsAddingMandatoryAction" containing a peer entries
     * in the way of: The entry is stored in the specific data.properties file
     * for this particular procedure. where the content is expressed in the way
     * of fieldNAmes between spaces where the first field is the one having the
     * prerrequisites. and all different fields separated by pipe, "|". Example:
     * project_fieldsAddingMandatoryInsert:analysis method_name
     * method_version*analysis method_name method_version|method_name*analysis
     * method_name method_version|spec*spec spec_code spec_code_version
    ...
    /**
     * When logging/creating objects that conceptually requires one template to
     * define its nature then one call to this method is required to get all
     * those parameters and log/create the new instance accordingly.Each
     * procedure has a specific parameter field called in the way of
     * "procedureName-config" containing a peer entries in the way of:
     * tableName__configTableName = Specify the table name where the template is
     * stored. tableName_configTableKeyFields = Specify the mandatory fields
     * that should be present in the peer fieldNames/fieldValues to link the new
     * object with its template in the proper and expected way.
     *
     * @param procInstanceName
     * @param fieldNames - Fields for the filter to find and get the proper
     * template.
     * @param fieldValues - Values for the fields described above.
     * @param tableName. Table where the template is stored in.
     * @return String[] when position 3 is set to FALSE then the template is not
     * found.
     */
    public static Object[] configObjectExists(String procInstanceName, String[] fieldNames, Object[] fieldValues, String tableName) {
        ...
        for (Integer i = 0; i < configTableKeyFieldName.length; i++) {
            ...
            if (fieldPosic == -1) {
                if (missingFieldInArray.length() > 0) {
                    missingFieldInArray = missingFieldInArray.append(", ");
                }
    ...
    /**
     * In some cases the field value requires check a kind of logic to verify
     * that the value is aligned with a particular business rule.When this is
     * required then a peer should be added to the properties field Each
     * procedure has a specific parameter field called in the way of
     * "procedureName-config" containing a peer entries in the way of:
     * tableName+"_specialFieldsCheck - Specify the field having this need.
     * tableName+"_specialFieldsCheck_methodName - The method to be invoked that
     * contains the logic.
     *
     * @param procInstanceName - String - Procedure
     * @param areaName
     * @param fieldNames - String[] - fields involved in the actionName being
     * performed
     * @param fieldValues - Object[] - field values
     * @param tableName - String - Table Name
     * @param actionName - String - action being performed
     * @return String[] - Returns detailed info about the evaluation and where
     * it ends, position 3 set to TRUE means all is ok otherwise FALSE.
     */
    ...
    /**
     * trapMessage is the method that should reduce the lines of code for
     * justifying lines of code against its requirement to keep the track about
     * which is the requirement covered by each section in each method. When
     * running the code in Dev-Mode then it should mark as 'covered' the
     * existing requirement or create one record for this given requirement The
