...
import module.formulation.logic.DataFormulation;
...
public class ClassFormulation implements ActionsClass{
    ...
    public ClassFormulation(HttpServletRequest request, FormulationAPIactionsEndpoints endPoint) {
...
*/
        ...
        DataFormulation formulaObj = null;
        String formulaName = "";
        if (Boolean.FALSE.equals("FORMULAAUDIT_SET_AUDIT_ID_REVIEWED".equalsIgnoreCase(endPoint.getName()))) {
            formulaName = argValues[0].toString();
        }
        if (Boolean.FALSE.equals("NEW_FORMULA".equalsIgnoreCase(endPoint.getName())) 
            && Boolean.FALSE.equals(endPoint.getName().toUpperCase().startsWith("CONFIG_"))
            && Boolean.FALSE.equals("FORMULAAUDIT_SET_AUDIT_ID_REVIEWED".equalsIgnoreCase(endPoint.getName()))                
            ) {
            formulaObj = new DataFormulation(formulaName);
            if (Boolean.TRUE.equals(formulaObj.getHasError())) {
                this.actionDiagnosesObj = formulaObj.getErrorDetail();
                this.relatedObj = rObj;
                rObj.killInstance();
                formulaObj = null;
                return;
        ...
        switch (endPoint) {
            case NEW_FORMULA:
                ...
                String ingredientsList = LPNulls.replaceNull(argValues[4]).toString();
                ...
                if (purpose.length()>0){
                    fieldNames = LPArray.addValueToArray1D(fieldNames, TblsFormulationData.Formula.PURPOSE.getName());
                    fieldValues = LPArray.addValueToArray1D(fieldValues, purpose);                    
                }
                actionDiagnoses = DataFormulation.createNewFormula(formulaName, projectName, fieldNames, fieldValues, ingredientsList);
                break;
