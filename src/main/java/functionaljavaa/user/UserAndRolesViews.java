...
import lbplanet.utilities.LPLdap;
...
public class UserAndRolesViews {
    ...
    /**
     * Returns Object[] LABPLANET_FALSE when the person not found or user name
     * in position 0.
     *
     * @param userName
     * @return
    ...
    /**
     * This method makes no sense once the Rdbms instance is created once by
     * singleton pattern <br>
     * This method would be replaced by checking user and password against the
     * info in the token
     *
     * @param user
     * @param pass
    ...
    public static final Object[] isValidUserPassword(String user, String pass) {
        ...
        EnumIntTableFields[] fieldsToRetrieve = EnumIntTableFields.getTableFieldsFromString(TblsApp.TablesApp.USERS,
            new String[]{Users.PASSWORD.getName(), Users.USES_LDAP.getName(), Users.LDAP_SETTING.getName(), 
                Users.LDAP_USER_NAME.getName(), Users.LDAP_DN.getName()});
        ...
        if (Boolean.TRUE.equals(Boolean.valueOf(LPNulls.replaceNull(tableData[0][1]).toString()))) {
            if (Boolean.TRUE.equals(LPLdap.LdapValidateUser(LPNulls.replaceNull(tableData[0][2]).toString(), LPNulls.replaceNull(tableData[0][3]).toString()
                , pass, LPNulls.replaceNull(tableData[0][4]).toString()))){
    ...
    public static final Object[] createAppUser(String uName, String[] fldNames, Object[] fldValues) {
        ...
        if (Boolean.FALSE.equals(LPPlatform.LAB_FALSE.equalsIgnoreCase(personByUserObj[0].toString()))) {
            return ApiMessageReturn.trapMessage(LPPlatform.LAB_FALSE, "UserAlreadyExists", new Object[]{uName});
        }
        Object[] personIdDiagn = getNextAppPersonId();
        if (LPPlatform.LAB_FALSE.equalsIgnoreCase(personIdDiagn[0].toString())) {
            return personIdDiagn;
        }
        ...
        if (Boolean.FALSE.equals(personCreatedDiagn.getRunSuccess())) {
            return personCreatedDiagn.getApiMessage();
        }
