...
import com.labplanet.servicios.app.IncidentAPI;
...
public class AppIncidentEnums {
    ...
    public enum IncidentAPIEndpoints{
        ...
        NEW_INCIDENT("NEW_INCIDENT", "incidentTitle|incidentDetail", "", "incidentNewIncident_success",  
            new LPAPIArguments[]{ new LPAPIArguments(IncidentAPI.ParamsList.INCIDENT_TITLE.getParamName(), LPAPIArguments.ArgumentType.STRING.toString(), true, 6 ),
                new LPAPIArguments(IncidentAPI.ParamsList.INCIDENT_DETAIL.getParamName(), LPAPIArguments.ArgumentType.STRING.toString(), true, 7 ), },
...
/*                new JSONArray()
                    ...
                    .add(new JSONObject("table",TblsApp.Incident.TBL.getName()))),*/
        CONFIRM_INCIDENT("CONFIRM_INCIDENT", COMMON_PARAMS, "", "incidentConfirmIncident_success",  
            new LPAPIArguments[]{ new LPAPIArguments(IncidentAPI.ParamsList.INCIDENT_ID.getParamName(), LPAPIArguments.ArgumentType.INTEGER.toString(), true, 6 ),
                new LPAPIArguments(IncidentAPI.ParamsList.NOTE.getParamName(), LPAPIArguments.ArgumentType.STRING.toString(), true, 7)},
            Json.createArrayBuilder().add(Json.createObjectBuilder().add("repository", GlobalVariables.Schemas.APP.getName())
                .add("table", TblsApp.Incident.TBL.getName()).build()).build()
                ),
        ...
        ADD_NOTE_INCIDENT("ADD_NOTE_INCIDENT", COMMON_PARAMS, "", "incidentAddNoteToIncident_success",  
            new LPAPIArguments[]{ new LPAPIArguments(IncidentAPI.ParamsList.INCIDENT_ID.getParamName(), LPAPIArguments.ArgumentType.INTEGER.toString(), true, 6 ),
                new LPAPIArguments(IncidentAPI.ParamsList.NOTE.getParamName(), LPAPIArguments.ArgumentType.STRING.toString(), true, 7),
            new LPAPIArguments(IncidentAPI.ParamsList.NEW_STATUS.getParamName(), LPAPIArguments.ArgumentType.STRING.toString(), false, 7)},
            Json.createArrayBuilder().add(Json.createObjectBuilder().add("repository", GlobalVariables.Schemas.APP.getName())
                .add("table", TblsApp.Incident.TBL.getName()).build()).build()
                ),
        ;
