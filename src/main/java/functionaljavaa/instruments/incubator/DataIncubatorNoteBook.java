...
//        INCUBATOR_NOT_EXIST("IncubatorNotExists", "The incubator <*1*> does not exist for procedure <*2*>", "La incubadora <*1*> no existe para el proceso <*2*>"),
    ...
    private enum EventType {
        TEMPERATURE_READING, ACTIVATE, DEACTIVATE
    }
    ...
    public static InternalMessage newTemperatureReading(String instName, String personName, BigDecimal temperature) {
        ...
        if (LPPlatform.LAB_TRUE.equalsIgnoreCase(Rdbms.dbTableExists(procInstanceName, LPPlatform.buildSchemaName(procInstanceName, GlobalVariables.Schemas.CONFIG.getName()), TblsEnvMonitConfig.TablesEnvMonitConfig.INSTRUMENT_INCUBATOR.getTableName(), TblsEnvMonitConfig.InstrIncubator.LOCKED.getName())[0].toString())) {
    ...
    /**
     * This method returns the last temperature reading for the latest activate
     * session, in other words, when invoked after having two temperature
     * readings after being activated from a deactivation period then it will
     * return just those two readings even not being enough to covers the
    ...
    public static Object[][] getLastTemperatureReading(String instName, Integer points, LocalDateTime startDate, LocalDateTime endDate, String[] fieldsToRetrieve, Boolean withMask) {
        ...
        if (procInstanceName == null) {
            return LPArray.array1dTo2d(new String[]{LPPlatform.LAB_FALSE}, 1);
        }
        Object[][] instrInfo = null;
        if (Boolean.TRUE.equals(withMask)) {
        ...
        if (Boolean.FALSE.equals(Boolean.valueOf(instrInfo[0][activeFldPosic].toString()))) {
        ...
        for (Object[] currReading : instrNotebook) {
            ...
            if ((EventType.ACTIVATE.toString().equalsIgnoreCase(currEventType)) || (EventType.DEACTIVATE.toString().equalsIgnoreCase(currEventType))
                    || (EventType.TEMPERATURE_READING.toString().equalsIgnoreCase(currEventType) && (endDate == null) && (points != null) && (points <= pointsAdded))) {
                break;
            }
    ...
    public static Object[] incubatorLocking(String instName, Object[] specEvalInfo, String[] incubFldNames, Object[] incubFldValues) {
        ...
        if (DataSampleStructureSuccess.EVALUATION_IN.getErrorCode().equalsIgnoreCase(specEvalInfo[0].toString())) {
            ...
            if (specEvalInfo[0].toString().toUpperCase().contains("SPEC")
                    && (Boolean.FALSE.equals("TRUE".equalsIgnoreCase(LPNulls.replaceNull(incubFldValues[LPArray.valuePosicInArray(incubFldNames, TblsEnvMonitConfig.InstrIncubator.LOCKED.getName())].toString()))))) {
                updFldName = new String[]{TblsEnvMonitConfig.InstrIncubator.LOCKED.getName(), TblsEnvMonitConfig.InstrIncubator.LOCKED_REASON.getName()};
        ...
        if (updateRecordFieldsByFilter == null) {
            return ApiMessageReturn.trapMessage(LPPlatform.LAB_TRUE, "noChangesRequired", null);
        }
