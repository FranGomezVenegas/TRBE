    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            for (Integer i = 0; i < configSamplingPlanTestingArray.length; i++) {
                ...
                if (configSamplingPlanTestingArray[i][0] != null) {
                    procInstanceName = configSamplingPlanTestingArray[i][0].toString();
                }
                if (configSamplingPlanTestingArray[i][5] != null) {
                    actionName = configSamplingPlanTestingArray[i][5].toString();
                }
                ...
                switch (LPNulls.replaceNull(actionName).toUpperCase()) {
                    case "NEWSAMPLINGDETAIL":
                        if (configSamplingPlanTestingArray[i][1] != null) {
                            procInstanceName = configSamplingPlanTestingArray[i][0].toString();
                        }
                        if (configSamplingPlanTestingArray[i][1] != null) {
                            userName = configSamplingPlanTestingArray[i][2].toString();
                        }
                        if (configSamplingPlanTestingArray[i][3] != null) {
                            fieldName = (String[]) configSamplingPlanTestingArray[i][3];
                        }
                        if (configSamplingPlanTestingArray[i][4] != null) {
                            fieldValue = (Object[]) configSamplingPlanTestingArray[i][4];
                        }
