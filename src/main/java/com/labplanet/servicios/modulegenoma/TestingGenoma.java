...
import functionaljavaa.testingscripts.StepExecDetail;
...
public class TestingGenoma extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Object[][] testingContent = ts.getScriptSteps().getOriginalSteps();
            for (Integer iLines = numHeaderLines; iLines < testingContent.length; iLines++) {
                ...
                fileContentTable1Builder.append(LPTestingOutFormat.rowAddFields(
                        new Object[]{iLines - numHeaderLines + 1, LPNulls.replaceNull(testingContent[iLines][5]).toString()}));

                StepExecDetail stExec = new StepExecDetail(iLines + 1, iLines, testingContent[iLines], numEvaluationArguments, ts.getScriptId(), actionName.toString());
                ts.getTstAssertSummary().increaseTotalTests();
                Object[] argsWithNamesAndValues = null;
                ...
                if (Boolean.TRUE.equals(clssProjectController.getFunctionFound())) {
                    ...
                    argsWithNamesAndValues = clssProjectController.getArgsWithNamesAndValues();
                    ...
                    if (Boolean.TRUE.equals(clssStudyController.getFunctionFound())) {
                        ...
                        argsWithNamesAndValues = clssStudyController.getArgsWithNamesAndValues();
                        testingContent[iLines][testingContent[0].length - 1] = functionRelatedObjects;
                        fileContentTable1Builder.append(clssStudyController.getRowArgsRows());
                    }
                }
                stExec.setFunctionEvaluation(functionEvaluation, argsWithNamesAndValues);
                if (testingContent[iLines][0] == null) {
                    ts.getTstAssertSummary().increasetotalLabPlanetBooleanUndefined();
                }
                if (testingContent[iLines][1] == null) {
                    ts.getTstAssertSummary().increasetotalLabPlanetErrorCodeUndefined();
                }
                fileContentTable1Builder.append(LPTestingOutFormat.rowAddFields(
                        new Object[]{
                ...
                stExec.setFunctionEvaluation(functionEvaluation, argsWithNamesAndValues);
                // ******* stExec.saveInDbStepEval !!!!! ESTO SE PUEDE MOVER A UN UPDATE MASIVO Y DELEGARLO A TESTINGSRIPT !
                stExec.saveInDbStepEval(functionEvaluation, functionRelatedObjects, stExec.getTstAssert(), ts.getTestingSource());

                ts.getScriptSteps().addStep(iLines, stExec);

                stExec.evaluationIfInterrupts(ts.getTstOut(), testingContent.length);
                if (Boolean.TRUE.equals(stExec.getInterruptExecution())) {
                    ts.runReport(request, response, stExec.getInterruptExecutionReason());
                    return;
                }
            }
            ts.runReport(request, response, null);
